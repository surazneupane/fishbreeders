                ...
                <div class="md:grid md:grid-cols-3 md:gap-6">
                    <div class="md:col-span-1">
                        <div class="px-4 sm:px-0">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ _('Edit Forum Category') }}</h3>
                            <p class="mt-1 text-sm text-gray-600">
                              <strong> {{$forumCategory->title}} </strong>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 md:mt-0 md:col-span-2">
                     ...
                     <p style="color: green">   {{Session::get('success')}} </p>
                        @endif
                        <form action="{{ route('forumcategory.update', $forumCategory->id) }}" method="POST">
                            @csrf
                            @method('put')
                            <input type="text" hidden name="id" id="id" value="{{ $forumCategory->id }}" />
                                ...
                                <div class="px-4 py-5 bg-white sm:p-6">
                                    ...
                                    <div class="col-span-6 sm:col-span-2">
                                        ...
                                        <select id="status" name="status" autocomplete="status" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm
                                                ...
                                                ">
                                            <option value="1" {{ $forumCategory->status == "1" ? 'selected' : '' }}>Active
                                            </option>
                                            <option value="0" {{ $forumCategory->status == "0" ? 'selected' : '' }}>In-Active
                                            </option>
                                        </select>
                                        @error('status')
                                        <span class="text-sm text-red-500">
